...
function video_popup_ajax_result_on_pageload(){
	if( isset($_GET['vp_on_pageload']) and get_option('vp_al_op_video_url') ){
		...
		<div style="max-width:880px; height:440px;" class="vp-flex <?php echo $remove_border.$video_type_class; ?>">
			...
			<?php if ( preg_match("/(youtube.com)|(youtu.be)/", $video_url) ) : ?>
				<iframe src="https://www.youtube.com/embed/<?php echo $the_video_id; ?>?autoplay=<?php echo $autoplay; ?>" allowfullscreen="true" allow="autoplay"></iframe>
			<?php endif; ?>
...
add_action('init', 'video_popup_on_pageload');


function video_popup_html_on_pageload(){
	if( get_option('vp_al_op_video_url') ){
		...
		if( !isset($_COOKIE["video_popup_ct_$cookie_id"]) or !get_option('vp_al_op_cookie') ){
			...
			or get_option( 'vp_al_op_display' ) == 'custom' and ( $single === true or $page === true )
				...
				?>
					...
					<div class="YouTubePopUp-Wrap" style="display: none;">
						<div class="Video-PopUp-Content vp-on-pageload-wrap">
						</div>
					</div>
				<?php
				}
...
add_action('wp_footer', 'video_popup_html_on_pageload');
