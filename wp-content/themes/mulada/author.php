	...
	<?php mulada_site_sub_content_start(); ?>
		<?php mulada_container_before(); ?>
			...
			<section class="category-archive-title">
				<h1><?php $allowed_html = array ( 'span' => array() ); wp_kses (printf( __( 'All Posts By <span>%s</span>', 'mulada' ), '' . get_the_author() . '' ) , $allowed_html ); ?></h1>
			</section>
			<?php mulada_post_bottom_author_info(); ?>
			<?php mulada_row_before(); ?>
				<?php mulada_content_area_start(); ?>
					<?php if ( have_posts() ) : ?>
						<div class="category-post-list post-list" id="posts">
